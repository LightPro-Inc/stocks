...
public class OperationTypeRs extends StocksBaseRs {
		...
		return createHttpResponse(
				new Callable<Response>(){
					...
					public Response call() throws IOException {
						...
						item.update(cmd.name(), source, destination, cmd.category(), sequence, defaultPreparationOpType, defaultReturnOpType);
						
						log.info(String.format("Mise à jour des données du type d'opération %s", cmd.name()));
		...
		return createHttpResponse(
				new Callable<Response>(){
					...
					public Response call() throws IOException {
						...
						log.info(String.format("Suppression du type d'opération %s", item.name()));
						return Response.status(Response.Status.OK).build();
					}
				});	
	}
	...
	@GET
	@Secured
	@Path("{id}/operation/todo")
	@Produces({MediaType.APPLICATION_JSON})
	public Response getTodoOperations(@PathParam("id") UUID id) throws IOException {	
		...
		return createHttpResponse(
				new Callable<Response>(){
					...
					public Response call() throws IOException {
						...
						List<OperationVm> items = stocks().operationTypes().get(id).todoOperations().all()
																		 .stream()
																 		 .map(m -> new OperationVm(m))
																 		 .collect(Collectors.toList());
		...
		return createHttpResponse(
				new Callable<Response>(){
					...
					public Response call() throws IOException {
						...
						Operation operation = type.addOperation(cmd.documentSource(), cmd.sourceLocationId(), cmd.destinationLocationId(), cmd.movementDate(), cmd.delayed(), partner);
						...
						log.info(String.format("Création de l'opération N° %s", operation.reference()));
